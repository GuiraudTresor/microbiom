...
float <- function(val) ifelse(!grepl("\\d+.\\d+",val),sprintf("%.3f",val),val)
...
bacdat <- function(ID){
  ...
  df <- data.frame(
    ...
    Temp_optimum = ifelse(paste0(ID,".Culture and growth conditions.culture temp") %in% names(z),
                          grep("optimum",z[[paste0(ID,".Culture and growth conditions.culture temp")]]),NA),
      row.names = NULL,stringsAsFactors = FALSE,check.names = TRUE)
  df <- df %>%
    mutate(Temp_optimum = ifelse(!is.na(Temp_optimum),z[[paste0(ID,".Culture and growth conditions.culture temp.","[[",Temp_optimum,"]]",".temperature")]],NA)) 
    
  return(df)
}

#z[[paste0(ID,".Culture and growth conditions.culture temp.","[[",grep("optimum",paste0(ID,".Culture and growth conditions.culture temp")),"]]",".temperature")]],NA)
